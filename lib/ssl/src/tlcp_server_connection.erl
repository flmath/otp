    ...
    case Versions of
        [?TLS_1_3] ->
            Connection:next_event(start, no_record, State0#state{recv = Recv#recv{from = From}},
                                  [{change_callback_module, tlcp_server_connection_1_3},
...
initial_hello(Type, Event, State) ->
    tls_dtlcp_server_connection:initial_hello(Type, Event, State).
...
hello(internal, #client_hello{client_version = ClientVersion} = Hello,
    ...
    try
        #state{ssl_options = SslOpts} = State1 =
            tls_dtlcp_server_connection:handle_sni_extension(State0, Hello),
...
gen_state(StateName, Type, Event, State) ->
    try tls_dtlcp_server_connection:StateName(Type, Event, State)
    catch throw:#alert{} = Alert ->
